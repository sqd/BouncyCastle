...
def address_in_net(ip: str, subnet: str) -> bool:
    ip_num = dotted_quad_to_num(ip)

    [subnet_address, subnet_bits] = subnet.split('/')
    subnet_address_num = dotted_quad_to_num(subnet_address)
    subnet_bits_num = int(float(subnet_bits))

    return ip_num & (0xFFFFffff << (32 - subnet_bits_num)) == subnet_address_num
...
def _matches_and_req(node: Node, reqs: AndRequirement) -> bool:
    for req in reqs.requirements:
        if not matches_requirements(node, req):
            return False
    return True


def _matches_or_req(node: Node, reqs: OrRequirement) -> bool:
    for req in reqs.requirements:
        if matches_requirements(node, req):
            return True
    return False


def _matches_subnet_req(node: Node, ip_subnet: str) -> bool:
    return address_in_net(node.last_known_address, ip_subnet)


def _matches_country_req(node: Node, country_code: int) -> bool:
    return node.country_code == country_code


def matches_requirements(node: Node, reqs: EndNodeRequirement) -> bool:
    if reqs.HasField('ip_subnet'):
        return _matches_subnet_req(node, reqs.ip_subnet)
    if reqs.HasField('country'):
        return _matches_country_req(node, reqs.country)
    if reqs.HasField('or'):
        return _matches_or_req(node, getattr(reqs, 'or'))
    if reqs.HasField('and'):
        return _matches_and_req(node, getattr(reqs, 'and'))
    if reqs.HasField('not'):
        return not matches_requirements(node, getattr(reqs, 'not'))
